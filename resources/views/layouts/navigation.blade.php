<nav x-data="{ open: false }" class="bg-white bg-opacity-90 backdrop-blur-md fixed w-full z-10 transition duration-300 shadow-md">
        ...
        <div class="flex justify-between items-center h-16">
            <div class="flex items-center space-x-4">
                ...
                <x-nav-link :href="route('supervisor.history')" 
                    class="text-gray-900 hover:bg-purple-500 hover:text-white transition px-4 py-2 rounded-lg {{ request()->routeIs('history') ? 'shadow-lg' : '' }}">
                    {{ __('History') }}
                </x-nav-link>
            ...
            <div class="hidden sm:flex sm:items-center sm:ml-auto space-x-2"> <!-- Changed to sm:ml-auto -->
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('home')" :active="request()->routeIs('home')" class="text-gray-900 dark:text-gray-100">
                {{ __('Home') }}
